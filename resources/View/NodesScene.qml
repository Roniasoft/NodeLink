...
I_NodesScene {
    ...
    * ****************************************************************************************/
    ...
    Keys.onDeletePressed: {
        var hasObjectsSelected = Object.keys(
                    scene.selectionModel.selectedModel).length > 0
    ...
    //! Change ScrollBars
    ScrollBar.horizontal: HorizontalScrollBar {//! Hide scrollbar when zoom process is running ...
    }

    ScrollBar.vertical: VerticalScrollBar {//! Hide scrollbar when zoom process is running ...
    }
    ...
    * ****************************************************************************************/
    ...
    ConfirmPopUp {
        ...
        confirmText: "Are you sure you want to delete "
                     + (Object.keys(
                            scene?.selectionModel?.selectedModel
                            ?? ({})).length > 1 ? "these items?" : "this item?")
    ...
    ContextMenu {
        ...
        scene: flickable.scene
        sceneSession: flickable.sceneSession
        nodePosition: Qt.vector2d(x + contentX,
                                  y + contentY).times(1 / flickableScale)
    }
    ...
    MouseArea {
        ...
        onReleased: function (mouse) {
            ...
            if (sceneSession.flickEnabled && (new Date()).getMilliseconds(
                        ) - elapsedTime < 5) {
                lastSpeed = Qt.vector2d(Math.max(-1, Math.min(1, lastSpeed.x)),
                                        Math.max(-1, Math.min(1, lastSpeed.y)))
                flickable.flick(lastSpeed.x * Screen.width,
                                lastSpeed.y * Screen.height)
                elapsedTime = 0
        ...
        onPositionChanged: function (event) {
            ...
            contentX = Math.max(0, Math.min(
                                    contentX + (lastPoint.x - event.x),
                                    flickable.contentWidth - flickable.width))
            contentY = Math.max(0, Math.min(
                                    contentY + (lastPoint.y - event.y),
    ...
    MouseArea {
        ...
        onClicked: mouse => {
                       if (mouse.button === Qt.LeftButton
                           ...
                           if (scene.selectionModel.lastSelectedObject(
                                   NLSpec.ObjectType.Link))
                           delTimer.start()
                       }
                   }
        ...
        onReleased: mouse => {
                        if (!wasDragged) {
                            ...
                            if (mouse.button === Qt.LeftButton) {
                                ...
                                //! Search for links under click point too
                                selectLinkAt(Qt.point(mouse.x, mouse.y),
                                             mouse.modifiers)
                            } else if (sceneSession.isSceneEditable
                                       && mouse.button === Qt.RightButton) {
                                contextMenu.popup(flickableContents.mapToItem(
                                                      flickable,
                                                      mouse.x, mouse.y))
                            }
                        }
        ...
        onDoubleClicked: mouse => {
                             ...
                             if (sceneSession.isSceneEditable) {
                                 ...
                                 scene.createCustomizeNode(
                                     scene.nodeRegistry.defaultNode,
                                     positionMapped.x, positionMapped.y)
                             }
                         }
    }
    ...
    MouseArea {
        ...
        onPositionChanged: mouse => {
                               scene.sceneGuiConfig._mousePosition = Qt.vector2d(
                                   mouse.x, mouse.y)
                           }
    ...
    Item {
        ...
        WheelHandler {
            ...
            onWheel: function (wheel) {
                zoomPoint = Qt.vector3d(
                            wheel.x - scene.sceneGuiConfig.contentX,
                            wheel.y - scene.sceneGuiConfig.contentY, 0)
                worldZoomPoint = flickableContents.mapToItem(
                            flickableContents.parent, wheel.x, wheel.y)
    ...
    Connections {
        ...
        function onZoomFactorChanged() {
            ...
            if (Math.abs(scene.sceneGuiConfig.zoomFactor
                         - sceneSession.zoomManager.zoomFactor) > 0.0001) {
        ...
        function onZoomToFitSignal() {
            ...
            Object.values(scene.nodes).forEach(node => {
                                                   ...
                                                   var rightNodeX = correctPosition.x
                                                   + node.guiConfig.width
                                                   var topNodeY = correctPosition.y
                                                   var bottomNodeY = correctPosition.y
                                                   + node.guiConfig.height

                                                   leftX = Math.min(leftX,
                                                                    leftNodeX)
                                                   rightX = Math.max(rightX,
                                                                     rightNodeX)
                                                   topY = Math.min(topY,
                                                                   topNodeY)
                                                   bottomY = Math.max(
            ...
            var zoomFactor = nodesLength > 1 ? Math.max(
                                                   sceneSession.zoomManager.minimumZoom,
                                                   Math.min(
                                                       sceneSession.zoomManager.maximumZoom,
                                                       widthRatio,
                                                       heightRatio)) : 1

            worldZoomPoint = Qt.vector2d((rightX + leftX) / 2,
                                         (topY + bottomY) / 2).times(
        ...
        function onZoomInSignal() {
            ...
            zoomPoint = Qt.vector3d(flickable.width / 2,
                                    flickable.height / 2, 0)
            worldZoomPoint = Qt.vector2d(
                        scene.sceneGuiConfig.contentX + flickable.width / 2,
        ...
        function onZoomNodeSignal(localZoomPoint: point, nodeView: Item, wheelAngle: int) {
            ...
            flickable.zoomPoint = Qt.vector3d(
                        localZoomPoint.x - scene.sceneGuiConfig.contentX,
                        localZoomPoint.y - scene.sceneGuiConfig.contentY, 0)
            flickable.worldZoomPoint = nodeView.mapToItem(
                        flickableContents.parent, localZoomPoint)
        ...
        function onResetZoomSignal(zoomFactor: real) {
            zoomPoint = Qt.vector3d(flickable.width / 2,
                                    flickable.height / 2, 0)
            worldZoomPoint = Qt.vector2d(
                        NLStyle.scene.defaultContentX + flickable.width / 2,
        ...
        function onZoomToNodeSignal(node: Node, targetZoomFactor: real) {
            ...
            var origin = Qt.vector2d(
                        node.guiConfig.position.x + node.guiConfig.width / 2,
    ...
    Connections {
        ...
        function onContentResizeRequested(diff: vector2d) {
            scene.sceneGuiConfig.contentWidth = Math.max(
                        NLStyle.scene.defaultContentWidth,
                        Math.min(NLStyle.scene.maximumContentWidth,
                                 scene.sceneGuiConfig.contentWidth + diff.x))
            scene.sceneGuiConfig.contentHeight = Math.max(
                        NLStyle.scene.defaultContentHeight,
    ...
    function findLink(gMouse): Link {
        ...
        Object.values(scene.links).forEach(obj => {
                                               var inputPos = obj.inputPort?._position
                                               ?? Qt.vector2d(0, 0)
                                               var outputPos = obj.outputPort?._position
                                               ?? Qt.vector2d(0, 0)
                                               if (Calculation.isPointOnLink(
                                                       gMouse.x, gMouse.y, 15,
                                                       obj.controlPoints,
                                                       obj.guiConfig.type)) {
                                                   foundLink = obj
                                               }
    ...
    function updateContentSize() {
        ...
        if (newContentWidth < flickable.width) {
            ...
            newContentWidth = scene.sceneGuiConfig.contentWidth
                    * sceneSession.zoomManager.zoomFactor
            newContentHeight = scene.sceneGuiConfig.contentHeight
                    * sceneSession.zoomManager.zoomFactor
        }

        if (newContentHeight < flickable.height) {
            //! Increase content height
            scene.sceneGuiConfig.contentHeight += (flickable.height - newContentHeight)
                    / sceneSession.zoomManager.zoomFactor
    ...
    function goToCenter() {
        ...
        var topLeftX = contentX * (1 / flickableScale)
                + (scene.sceneGuiConfig.sceneViewWidth * (1 / flickableScale))
                / 2 - (maxX - minX) / 2
        var topLeftY = contentY * (1 / flickableScale)
                + (scene.sceneGuiConfig.sceneViewHeight * (1 / flickableScale))
                / 2 - (maxY - minY) / 2
